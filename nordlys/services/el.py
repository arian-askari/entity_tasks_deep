    ...
    def __check_config(config):
        ...
        if config["method"] == "ltr":
            if config.get("model_file", None) is None:
                ...
                config["model_file"] = os.path.dirname(os.path.realpath(__file__))+"/../../data/el/model.txt"
        if config.get("kb_snapshot", None) is None:
            config["kb_snapshot"] = "data/el/snapshot_2015_10.txt"
            config["kb_snapshot"] = os.path.dirname(os.path.realpath(__file__))+"/../../data/el/snapshot_2015_10.txt"
        return config
