...
validations_report_path = os.path.join(base_path, "data" , "reports", "validation_reports_ModelThreeSteep.csv")
test_report_path = os.path.join(base_path, "data" , "reports" , "test_reports_ModelThreeSteep.csv")
trec_path = os.path.join(base_path, "qrels.test")
delimeter = "\t"
...
queries_path = os.path.join(base_path, 'queries_type_retrieval.json')
qrel_types_path = os.path.join(base_path, 'qrels-tti-dbpedia.txt')

##################################################################################################
train_set_row_path = os.path.join(base_path, 'train_set_row_sig17.csv')
train_set_feature_path = os.path.join(base_path, 'train_set_feature_sig17.csv')

types_unique_raw_path = os.path.join(base_path, 'types_unique_raw_sig17.csv')
queries_unique_raw_path = os.path.join(base_path, 'quries_unique_row_sig17.txt')

types_unique_feature_path = os.path.join(base_path, 'types_unique_feature_sig17.csv')
queries_unique_feature_path = os.path.join(base_path, 'quries_unique_feature_sig17.csv')

trainset_average_w2v_path = os.path.join(base_path, 'trainset_average_w2v_sig17.txt')

queries_w2v_char_level_path = os.path.join(base_path, 'queries_w2v_char_level_feature.csv')
q_ret_100_entities_path = os.path.join(base_path, 'q_ret_100_entities.csv')
entity_unique_avg_w2v_path = os.path.join(base_path, 'entity_unique_avg_w2v.json')

trainset_translation_matrix_path = os.path.join(base_path, 'trainset_translation_matrix.txt')

trainset_translation_matrix_detail_path = os.path.join(base_path, 'trainset_translation_matrix_score_e_detail_t_raw_')
trainset_translation_matrix_detail_normal_path = os.path.join(base_path,
                                                              'trainset_translation_matrix_score_e_detail_t_normal_')

trainset_translation_matrix_escore_path = os.path.join(base_path, 'trainset_translation_matrix_score_e_raw_')
trainset_translation_matrix_escore_normal_path = os.path.join(base_path, 'trainset_translation_matrix_score_e_normal_')

trainset_translation_matrix_cosine_typeDetail_path = os.path.join(base_path,
                                                                  'trainset_translation_matrix_cosine_typeDetail_raw_')
trainset_translation_matrix_cosine_typeDetail_normal_path = os.path.join(base_path,
                                                                         'trainset_translation_matrix_cosine_typeDetail_normal_')

###
type_terms_raw_path = os.path.join(base_path, 'types_unique_terms_sig17.csv')
type_terms_unique_w2v_path = os.path.join(base_path, 'type_terms_unique_w2v_path_sig17.csv')
trainset_type_terms_avg_q_avg_w2v_path = os.path.join(base_path,
                                                      'trainset_type_terms_avg_q_avg_w2v_sig17.txt')
###


type_tfidf_sorted_terms_raw_path = os.path.join(base_path, 'type_tfidf_sorted_terms_sig17.tsv')
type_dfs_sorted_raw_path = os.path.join(base_path, 'type_dfs_sorted_sig17.tsv')

type_w2v_char_level_tfidf_terms_sorted_path = os.path.join(base_path,
                                                           'type_w2v_char_level_tfidf_terms_sorted.json')
type_w2v_char_level_dfs_terms_sorted_path = os.path.join(base_path,
                                                         'type_w2v_char_level_dfs_terms_sorted.json')
trainset_translation_matrix_type_tfidf_terms_path = os.path.join(base_path,
                                                                 'trainset_translation_matrix_tfidf_terms')
trainset_translation_matrix_type_sdf_terms_path = os.path.join(base_path,
                                                               'trainset_translation_matrix_sdf_terms')

###

trainset_cosine_sim_average_w2v_path = os.path.join(base_path,
                                                    'trainset_cosine_sim_average_w2v_sig17.txt')

type_entity_cnt_path = os.path.join(base_path, 'types_details_light.tsv')
...
# entity_unique_word_level_w2v_path = os.path.join(dirname, 'entity_unique_word_level_w2v_k')
entity_unique_word_level_w2v_path = os.path.join(base_path, 'entity_unique_word_level_w2v_k')
print(entity_unique_word_level_w2v_path)
# sys.exit(1)
# trainset_translation_matrix_3d_path = os.path.join(dirname, 'trainset_translation_matrix_3d_')
trainset_translation_matrix_3d_path = os.path.join(base_path, 'trainset_translation_matrix_3d_')
...
word2vec_train_set_path = os.path.join(base_path, 'GoogleNews-vectors-negative300.bin')
...
def load_trainset_average_w2v():
    print("load_trainset_average_w2v")
...
def get_trainslation_matrix_score_e(q_id, type, queries_w2v_char_level_dict, queries_ret_100_entities_dict,
    ...
    for q_w2v_word in q_w2v_words:
        ...
        for retrieved in q_retrieved_entities[:k]:
            ...
            row_np_cosine_typedetail[current_column] = cosine_sim * (
            math.log10(COUNT_ENTITES_OF_TYPES / float(type_ent_cnt_dict[type][0])))
    ...
    translation_matrix_np_zscore_normal = get_normalize2D(translation_matrix_np)
    translation_matrix_e_score_zscore_normal = get_normalize2D(translation_matrix_e_score)
    translation_matrix_cosine_typeDetail_zscore_normal = get_normalize2D(
        translation_matrix_cosine_typeDetail)
    ...
    return (translation_matrix_np, translation_matrix_np_zscore_normal, translation_matrix_e_score
            ...
            translation_matrix_cosine_typeDetail_zscore_normal)
...
def save_translation_matrix_entity_score(k=100):
    ...
    with open(train_set_row_path) as tsv:
        ...
        json.dump(train_set_translation_matrix_normal_zscore_dict,
                  fp=open(trainset_translation_matrix_detail_normal_path + str(k) + ".json", 'w'))
        json.dump(train_set_translation_matrix_e_score_normal_zscore_dict,
                  fp=open(trainset_translation_matrix_escore_normal_path + str(k) + ".json", 'w'))
        json.dump(train_set_translation_matrix_dict,
                  fp=open(trainset_translation_matrix_detail_path + str(k) + ".json", 'w'))
        json.dump(train_set_translation_matrix_e_score_dict,
                  fp=open(trainset_translation_matrix_escore_path + str(k) + ".json", 'w'))

        json.dump(train_set_translation_matrix_cosine_typeDetail_dict,
                  fp=open(trainset_translation_matrix_cosine_typeDetail_path + str(k) + ".json", 'w'))
        json.dump(train_set_translation_matrix_cosine_typeDetail_normal_zscore_dict,
                  fp=open(trainset_translation_matrix_cosine_typeDetail_normal_path + str(k) + ".json", 'w'))
...
def get_entity_word_level_w2v(e_abstract, k=100):
    ...
    if type(e_abstract) is not list:
        tokens = e_abstract.split(
            " ")  # abstract is preprossed in entity_retrieval funciton in uitls, so split by space is enough
    else:
        ...
        for q_id, q_ret in q_rel_entities_dict.items():
            for ret in q_ret:
                ...
                if retrieved_entity not in entity_word_level_w2v_dict:
                    if use_tfidf == False:
                        entity_word_level_w2v_dict[retrieved_entity] = get_entity_word_level_w2v(abstract,
                                                                                                 k=top_k).tolist()
                    else:
                        tokens = [token[0] for token in abstract_tf_idf_sorted]
                        entity_word_level_w2v_dict[retrieved_entity] = get_entity_word_level_w2v(tokens,
                                                                                                 k=top_k).tolist()

    if use_tfidf == False:
        json.dump(entity_word_level_w2v_dict, fp=open(entity_unique_word_level_w2v_path + str(top_k) + ".json", 'w'))
    else:
        json.dump(entity_word_level_w2v_dict,
                  fp=open(entity_unique_word_level_w2v_path + str(top_k) + "_tfidf.json", 'w'))
...
def save_translation_matrix_entity_3d(top_entities=20, top_k_term_per_entity=50, use_tfidf=False):
    ...
    entity_unique_word_level_w2v_dict = get_entity_unique_word_level_w2v(top_k_term=top_k_term_per_entity,
                                                                         use_tfidf=use_tfidf)
    ...
    with open(train_set_row_path) as tsv:
        for line in csv.reader(tsv, dialect="excel-tab"):  # can also
            ...
            translation_matrix_np = get_trainslation_matrix_3d(q_id=q_id, type=q_type,
                                                               ...
                                                               type_ent_cnt_dict=type_ent_cnt_dict,
                                                               top_entities=top_entities,
                                                               top_k_term=top_k_term_per_entity)
...
def get_trainset_translation_matrix3d(top_entities=20, top_k_term_per_entity=50):
    path = trainset_translation_matrix_3d_path + "tope(" + str(top_entities) + "topterm(" + str(
        top_k_term_per_entity) + ".json"
...
def get_train_test_data_translation_matrix_3d(queries_for_train, queries_for_test_set, top_entities=20,
                                              top_k_term_per_entity=50, use_tfidf=False):
    global trainset_average_w2v_path
    tmp = get_train_test_data_translation_matrix_3d_pat(top_entities=top_entities,
                                                        top_k_term_per_entity=top_k_term_per_entity,
                                                        use_tfidf=use_tfidf)
    print(tmp)
    if trainset_average_w2v_path != tmp:
        global trainset_average_w2v
...
def get_train_test_data_translation_matrix_3d_pat(top_entities=20, top_k_term_per_entity=50, use_tfidf=False):
    path = ""
    if use_tfidf == False:
        path = trainset_translation_matrix_3d_path + "tope(" + str(top_entities) + "topterm(" + str(
            top_k_term_per_entity) + ".json"
        ...
        path = trainset_translation_matrix_3d_path + "tope(" + str(top_entities) + "topterm(" + str(
            top_k_term_per_entity) + "_tfidf.json"
    return path
...
# global trainset_average_w2v_path
# tmp = get_train_test_data_translation_matrix_3d_pat(top_entities=100, top_k_term_per_entity=50, use_tfidf=False)
# trainset_average_w2v_path = tmp
# load_trainset_average_w2v()
...
import trec_output as trec
import file_utils


from model_generator_EntityScore_3d import Model_Generator
base_path = '/content/gdrive/My Drive/Colab/'

queries_path = os.path.join(base_path, "queries_type_retrieval.json")
models_path = os.path.join(base_path, "data", "runs", "")
models_path_from_root = os.path.join(base_path,"data", "runs", "")

results_path_from_root = os.path.join(base_path ,"data", "results", "")
...
def nested_cross_fold_validation():
    ...
    with open(queries_path, 'r') as ff:
        ...
        for i in range(k_fold):
            ...
            for layers, activation in zip(layers_for_evaluate, activation_for_evaluate):
                for dropout_rate in dropout_rates:
                    ...
                    for j in range(k_fold - 1):
                        ...
                        train_X, train_Y, test_X, test_Y_one_hot, q_id_test_list, test_TYPES, test_Y = get_train_test_data_translation_matrix_3d(queries_train_set, queries_validation_set, top_entities=top_entities, top_k_term_per_entity=top_k_term_per_entity, use_tfidf=use_tfidf)
            ...
            _, __, test_X, test_Y_one_hot, q_id_test_list, test_TYPES, test_Y = get_train_test_data_translation_matrix_3d(queries_for_train, queries_for_test_set, top_entities=top_entities, top_k_term_per_entity=top_k_term_per_entity, use_tfidf = use_tfidf)
...
top_entities= 100 # age ok bood code jadid, 100 esh konam bebinam chi mishe !
