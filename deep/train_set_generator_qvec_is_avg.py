...
trainset_translation_matrix_type_tfidf_terms_path = os.path.join(dirname, '../data/types/sig17/trainset_translation_matrix_tfidf_terms_qavg')
...
def get_trainslation_matrix_type_terms(q_id, type, queries_w2v_char_level_dict, type_terms_k_top_char_level_w2v, k):
    ...
    if (all(v == 0 for v in q_w2v_words)):
        return translation_mattix_np #skip this row zeros, because query w2v doesn't exist !

    row_np = np.zeros(column_size)

    for w2c_type_term in t_term_w2c_words[:k]:
        current_column += 1
        cosine_sim = get_cosine_similarity(q_w2v_words, w2c_type_term)
