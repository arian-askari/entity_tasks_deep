...
class Model_Generator():
    """
    activation : [relu, relu, linear] means, first and second layers have activation relu, and third(output) layer have linear activation function
    layers: [1000,1000] means two layers, each layer have 1000 neurons
    optimizer: default value, adam
    ...
    def __init__(self, layers, activation=[], optimizer="adam", loss = "mse", learning_rate=0.0001,
        ...
        print("layers =", layers, ", activation =", activation, ", optimizer =", optimizer, ", loss =", loss,
              ...
              " ,dropout =", dropout, " ,csv_log_path =", csv_log_path)
    ...
    def fit(self, train_x, train_y, input_dim): #input_dim example: (600,)
        ...
        if len(self.__csv_log_path)>0:
            ...
            self.__history = self.__network.fit(train_x, train_y, epochs=self.__epochs, batch_size=self.__batch_size, verbose=2, callbacks=[csv_logger])
        else:
            self.__history = self.__network.fit(train_x, train_y, epochs=self.__epochs, batch_size=self.__batch_size, verbose=2)
