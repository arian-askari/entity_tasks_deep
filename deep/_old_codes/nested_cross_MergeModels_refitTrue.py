...
from deep._old_codes.model_generator_MergeModels_TwoStep import Model_Generator
...
def nested_cross_fold_validation():
    ...
    with open(queries_path, 'r') as ff:
        ...
        for i in range(k_fold):
            ...
            #                                              acc_train, loss_validation, acc_validation, abs(loss_train-loss_validation), float(n5_on_this_fold)))
            #
            ...
            #arian
            """ ma ghablan best model ra peyda karde im, hal faghat mikhaahiiiim ba un best parameter haiii ke peyda kardeim k cross fold validation bezanim"""
            train_X_EC, train_Y_EC, test_X_EC, test_Y_EC_one_hot_EC, q_id_test_list, test_TYPES_EC, test_Y_EC = tsg_EC.get_train_test_data_translation_matrix_3d(
                queries_for_train, queries_for_test_set, top_entities=top_entities,
                top_k_term_per_entity=top_k_term_per_entity, use_tfidf=use_tfidf)

            train_X_TC, train_Y_TC, test_X_TC, test_Y_TC_one_hot_TC, q_id_test_list, test_TYPES_TC, test_Y_TC = tsg.get_train_test_data_translation_matric_type_centric(
                queries_for_train, queries_for_test_set, k=50)

            model = Model_Generator(layers=[100,1], activation=['relu','linear'], epochs=100, dropout=0,
                                    category="regression", learning_rate=0.0001, loss="mse",
                                    batch_size=128,
                                    optimizer="adam", top_k=50)  # regression sample

            train_Y_EC = np.argmax(train_Y_EC, axis=1)
            train_Y_TC = np.argmax(train_Y_TC, axis=1)

            model.fit(train_xTC=train_X_TC, trainxEC=train_X_EC, train_y=train_Y_EC, input_dim=input_dim, test_x_TC=test_X_TC, test_x_EC=test_X_EC, test_y=test_Y_EC)
            ...
            report.append_test(
                ...
                , fold_outer_i=str(i)
                , loss_train_avg=str()
                , loss_validation_avg=str(tmp_val)
                , dropout=str(best_model.get_dropout())
                , loss_test=str(loss_test)
                , acc_test=str(loss_test)
                , acc_train_avg=str(tmp_val)
                , acc_validation_avg=str(tmp_val)
                , batch_size=str(best_model.get_batch_size())
                , epoch=str(best_model.get_epoch_count())
                , optimizer=str(best_model.get_optimizer())
                , loss_function=str(best_model.get_loss_function())
            ...
            loss_train_best_models_total = np.append(loss_train_best_models_total, float(tmp_val))
            acc_train_best_models_total = np.append(acc_train_best_models_total, float(tmp_val))

            loss_validation_best_models_total= np.append(loss_validation_best_models_total, float(tmp_val))
            acc_validation_best_models_total= np.append(acc_validation_best_models_total, float(tmp_val))
