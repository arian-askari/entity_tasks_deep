    ...
    with open(train_set_row_path) as tsv:
        for line in csv.reader(tsv, dialect="excel-tab"):  # can also
            ...
            if q_id not in train_set_translation_matrix_dict:
                ...
                if q_id == "INEX_LD-2009039" and q_type == "<dbo:Place>":
                    print(q_id, q_type)
                    print(translation_matrix_list)
                    jjjj = 0

                if q_id == "INEX_LD-2009039" and q_type == "<dbo:ArchitecturalStructure>":
                    print(q_id, q_type)
                    print(translation_matrix_list)

                    jjjj = 0

                if q_id == "INEX_LD-2009022" and q_type == "<dbo:Food>":
                    print(q_id, q_type)
                    print(translation_matrix_list)
...
def get_trainslation_matrix_score_e(q_id, type, queries_w2v_char_level_dict , queries_ret_100_entities_dict, entity_unique_avg_w2v_dict, type_ent_cnt_dict):
    ...
    if q_id == "INEX_LD-2009039" and type == "<dbo:Place>":
        print(q_id, type)
        print(translation_mattix_np.tolist())
        print("sum over all column[0]", translation_mattix_np[0].sum())
        jjjj = 0

    if q_id == "INEX_LD-2009039" and type == "<dbo:ArchitecturalStructure>":
        print(q_id, type)
        print(translation_mattix_np.tolist())
        print("sum over all column[0]", translation_mattix_np[0].sum())
        jjjj = 0

    if q_id == "INEX_LD-2009022" and type == "<dbo:Food>":
        print(q_id, type)
        print(translation_mattix_np.tolist())
        print("sum over all column[0]", translation_mattix_np[0].sum())
        jjjj = 0
